...
const todaStr = new Date().toISOString().split('T')[0]
...
export function useHabits(userId) {
    ...
    const isCompleted = (habitId, date = todaStr) => 
        completions.includes(`${habitId}:${date}`)

    const completionsForHabit = (habitId) => 
        completions.filter(c => c.startsWith(habitId + ':')).map(c => c.split(':')[1])

    const toggleHabit = async (habit) => {
        const done = isCompleted(habit.id)
        const key = `${habit.id}:${todaStr}`

        setCompletions(prev => done ? prev.filter(c => c !== key) : [...prev, key])
    ...
    if(done) {
        await supabase.from('habit_completions').delete().eq('habit_id', habit.id).eq('date', todaStr)
    } else {
    ...
    const addNote = async (text, topic) => {
        const {data, error} = await supabase.from('notes').insert({
            user_id: userId, topic, text, date: todaStr
        })
        .select()
        .single()
